...
|---|---|---|
| Frequency | Poisson GLM | `ClaimNb ~ BonusMalus + Area + VehPower + VehAge + DrivAge + VehBrand + VehGas + Region`, offset = log(Exposure) |
| Severity | Gamma GLM (log link) | Same predictors, fit only on policies with ≥1 claim, weighted by claim count |
